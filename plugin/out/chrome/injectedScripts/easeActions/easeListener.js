if (window.location.href.indexOf("https://ease.space") == 0 || window.location.href.indexOf("http://localhost:8080") == 0 || window.location.href.indexOf("https://localhost:8443/*") == 0 || window.location.href.indexOf("http://51.254.207.91/*") == 0) {
    ...
    extension.onMessage.addListener("logoutFrom", function logoutHandler(visitedWebsites, sendResponse) {
        document.dispatchEvent(new CustomEvent("LogoutFrom", {
            "detail": visitedWebsites
        }));
    });

    extension.onMessage.addListener("logoutDone", function logoutHandler(message, sendResponse) {
        document.dispatchEvent(new CustomEvent("LogoutDone", {
            "detail": message
        }));
    });
    ...
    document.addEventListener("NewConnection", function (event) {
        ...
        extension.sendMessage("NewConnection", {
