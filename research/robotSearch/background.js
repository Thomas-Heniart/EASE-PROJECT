...
function putResearch() {
    extension.storage.get("researchStorage", function(researchStorage) {
        var result = 0;
        var tot = 0;
        for (var i in researchStorage) {
            if (researchStorage[i]["online"] != {} && researchStorage[i]["offline"] != {}) {
                if (researchStorage[i]["online"].haveLogoutString && !researchStorage[i]["offline"].haveLogoutString) {
                    result++;                
                }
                tot++;
            }
        }
        console.log("Have logout string only on connected: " + result + "/" + tot);
        ...
        for (var i in researchStorage) {
            if (researchStorage[i]["online"] != {} && researchStorage[i]["offline"] != {}) {
                if (!researchStorage[i]["online"].haveInputPassword && researchStorage[i]["offline"].haveInputPassword) {
                    result++;                
                }
                tot++;
            }
        }
        console.log("Have input password only on deconnected: " + result + "/" + tot);
        ...
        for (var i in researchStorage) {
            if (researchStorage[i]["online"] != {} && researchStorage[i]["offline"] != {}) {
                if (!researchStorage[i]["online"].haveInputPassword && researchStorage[i]["offline"].haveInputPassword
                    && researchStorage[i]["online"].haveLogoutString && !researchStorage[i]["offline"].haveLogoutString) {
                    result++;               
                }
                tot++;
            }
        }
        console.log("Have all in same time: " + result + "/" + tot);

        result = 0;
        tot = 0;
        for (var i in researchStorage) {
            if (researchStorage[i]["online"] != {} && researchStorage[i]["offline"] != {}) {
                //console.log(researchStorage[i]["online"].cookies);
                if (researchStorage[i]["online"].cookies != researchStorage[i]["offline"].cookies) {
                    result++;
                }
                tot++;
            }
        }
        console.log("Cookies different on connected: " + result + "/" + tot);
    });
}