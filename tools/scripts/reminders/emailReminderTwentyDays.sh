...
function sendEmail() {
	...
	--data '{"personalizations": [{"to": [{"email":"'$email'", "name":"'$name'"}], "substitutions": {"username":"'$name'"}}], "from": {"email":"benjamin@ease.space", "name":"Benjamin"}, "template_id": "2c03ac41-648f-49c4-95dc-7057a09de38b"}'
}
...
WHERE DATE(last_connection) = DATE_SUB(CURDATE(), INTERVAL 20 DAY) AND websiteApps.type <> 'websiteApp' \                                                                                                          
