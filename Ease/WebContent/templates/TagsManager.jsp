	...
	<div>
		...
		<div>
			<form method="post" id="addTagForm" action="addTag">
				...
				<input type="submit" class="btn btn-default btn-primary" style="margin-left:25%; margin-top:1%; width:50%; text-align:center; position:relative;" value="Add tag" />
			</form>

		</div>
				
		<div style="margin-top:8%">
				<select id="websiteSelector" name="websiteToTag" onChange="showFormTags()" class="form-control" style="margin-left:25%; margin-top:1%; width:50%; position:relative;">
				...
				<div id="completeForm" style="display:none;">
					<div id="tagsContainer" style="margin-left:25%; margin-top:1%; width:50%; text-align:center; position:relative;">
					</div>
					<select id="tagSelector" name="websiteToTag" onChange="addTag()" class="form-control" style="margin-left:25%; margin-top:1%; width:50%; position:relative;">
					...
					<input type="submit" id="setTags" class="btn btn-default btn-primary" style="margin-left:25%; margin-top:1%; width:50%; text-align:center; position:relative;" value="Validate" />
				</div>
...
<script>
function enterTagsManagerMode() {
	$('#TagsManagerTab').addClass('show');
}

function leaveTagsManagerMode() {
	$('#TagsManagerTab').removeClass('show');
}
...
$(document).ready(function() {
	$('#TagsManagerTab #quit').click(function() {
...
function addTag(){
	...
	if(tagId!=0){
		if($("#tagsContainer div[tagId="+tagId+"]").length ==  0){
			...
			$("#tagsContainer").append('<div tagId="'+ tagId +'" class="btn-group tags-group" style="margin-left: 1px; margin-top: 3px;">'
	    			+'<a href="#" class="tag btn btn-default"'
					+'style="background-color: '+ color +'; border-color: '+ color +'; color: white;">'+ name
					+'</a>'
					+'<a href="#" onClick="deleteTag('+ tagId +')" class="btn btn-default delete-tag">X</a>'
	    		+'</div>');
		}
	}
}
