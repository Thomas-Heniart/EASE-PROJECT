...
Cookie 		fname = new Cookie("fname", Base64.getEncoder().encodeToString(user.getFirstName().getBytes(StandardCharsets.UTF_8)));
Cookie		lname = new Cookie("lname", Base64.getEncoder().encodeToString(user.getLastName().getBytes(StandardCharsets.UTF_8)));
...
<script>
	...
	function setupProfileSettings(profile) {
		...
		$(profile).find('.ProfileControlPanel #modifyNameForm .buttonSet #validate').click(function(){
			var index = $(this).closest('.owl-item').index();
			var name = $(this).closest('#modifyNameForm').find('#profileName').val();
			var button = $(this);
			var closeSectionButton = $(this).closest('.buttonSet').find('#cancel');
			...
			$.post(
				...
				function(data){
					...
					$(button).closest('#modifyNameForm').find('#profileName').val('');

					if (data[0] == 's'){
						$(closeSectionButton).click();
						$(button).closest('.ProfileBox').find('.ProfileName p').text('@' + name);
						$(button).closest('.profileSettingsTab').find('.sectionHeader .directInfo p').text(name);
		...
		$(profile).find('.ProfileControlPanel #modifyColorForm .buttonSet #validate').click(function(){
			var index = $(this).closest('.owl-item').index();
			var color = $(this).closest('#modifyColorForm').find('#color').val();
			var button = $(this);
			var closeSectionButton = $(this).closest('.buttonSet').find('#cancel');
			...
			$.post(
				...
				function(data){
					...
					if (data[0] == 's'){
						$(closeSectionButton).click();
						$(button).closest('.ProfileBox').find('.ProfileName').css('background-color', color);
						$(button).closest('.ProfileBox').attr('color', color);
						var string = '5px solid ' + color;
						$(button).closest('.ProfileBox').css('border-bottom', string);
						$(button).closest('.profileSettingsTab').find('#ColorSection .directInfo').css('background-color', color);
						showAlertPopup('Modifications successfully applied !', false);
					}else {
						if (data[0] != 'e'){
							document.location.reload(true);
		...
		$(profile).find('#deleteProfileForm .buttonSet #validate').click(function(){
			var idx;
			var name;
			var popup;
...
<script type="text/javascript">
	function checkForExtension(){
		...
		if (!($('#ease_extension').length)){
			...
			div.append($('<p class="info">Il ne te reste plus qu’à télécharger notre add-on !</p>'));
			div.append($('<button id="install-button">Install it !</button>'));
			$('.ProfilesView .owl-carousel').css('display', 'none');
			$('.ProfilesView').append(div);
			$('.ProfilesView').find('#install-button').click(function(){
...
<div class="ProfilesHandler">
	<img class="centeredItem" id="dragAndDropHelper" src="resources/other/Arrow.png" style="display: none;"/>
	<div class="owl-carousel">
		<c:forEach items='${profiles}' var="item">
		<div class="item" idx=<%= it%>>
			<div class="ProfileBox" style="border-bottom: 5px solid ${item.getColor()};" color="${item.getColor()}">
				<div class="ProfileName" style="background-color: ${item.getColor()};">
					<p>@<c:out value='${item.getName()}' /></p>
					<div class="ProfileSettingsButton"><i class="fa fa-fw fa-ellipsis-v"></i></div>
				</div>
				<div class="ProfileContent">
					<img class="Scaler" src="https://placehold.it/3x6" style="width: 100%; height: auto; visibility: hidden;"/>
					<div class=content>
						<div class="ProfileControlPanel" index=<%= it%>>
							<%it = it + 1; %>
							<div class="profileSettingsTab">
								<div class="sectionHeader" id="NameSection"><p class="title">Profile name</p><div class="directInfo"><p>${item.getName() }</p></div>			
							</div>
							<div class="sectionContent" id="contentName">
								<div   id="modifyNameForm">
									<input  id="profileName" name="profileName" type="text" placeholder="Profile name..."/>
									<div class="buttonSet">
							...
							<div class="sectionHeader" id="ColorSection"><p class="title">Color</p><div class="directInfo" style="background-color: ${item.getColor()}"></div>
						</div>
						<div class="sectionContent" id="contentColor">
							...
							<div   id="modifyColorForm">
								<div class="colorChooser">
									...
									<div class="lineColor">
										<%} %>
										<div class="color" color="${color.getColor()}" style="background-color: ${color.getColor()}"></div>
										<% itr++; %>
										<% if ((itr % 5) == 0) {%>
									</div>
									<%} %>
				...
				<div class="sectionHeader" id="DeleteProfilSection"><p class="title">Delete profile</p>
				</div>
		...
		<div class="SitesContainer">
			...
			<c:if test="${account.getType() eq 'Account'}">
				<div class="siteLinkBox" onclick="sendEvent(this)" login="${account.getLogin() }" webId="${account.getSite().getId()}" name="${account.getName()}">
			</c:if>
			<c:if test="${account.getType() eq 'LogWith'}">
				<div class="siteLinkBox" onclick="sendEvent(this)" webId="${account.getSite().getId()}" name="${account.getName()}">
			</c:if>			
				<div class="linkImage">
					<div class="deleteAppButton" onclick="showConfirmDeleteAppPopup(this, event)">
						<i class="fa fa-times"></i>
					</div>
					<div class="modifyAppButton" onclick="showModifyAppPopup(this, event)">
						<i class="fa fa-cog" aria-hidden="true"></i>
					</div>
					<img src="<c:out value='${account.getSite().getFolder()}logo.png'/>" />
				</div>
				<div class="siteName">
					<c:choose>
					<c:when test="${account.getName().length() > 14}">
					<p>${account.getName().substring(0,14)}...<p>
					</c:when>
					<c:otherwise>
					<p>${account.getName()}</p>					    
				</c:otherwise>
	...
	<div class="item" >
		<div class="AddProfileView">
			...
			<div class="scalerContainer">
				<img class="Scaler" src="https://placehold.it/3x6" style="width: 100%; height: auto; visibility: hidden;"/>
				<i class="fa fa-plus-circle centeredItem addHelper" aria-hidden="true"></i>
...
<div id="profileHelper" style="display: none">
	<div class="item">
		<div class="ProfileBox" style="border-bottom: 5px solid #35a7ff;" color="#35a7ff">
			...
			<div class="ProfileContent">
				...
				<div class="content">
					<div class="ProfileControlPanel" index="0">
						...
						<div class="profileSettingsTab">
							<div class="sectionHeader" id="NameSection"><p class="title">Profile name</p><div class="directInfo"><p>Default</p></div>			
						</div>
						<div class="sectionContent" id="contentName">
							<div id="modifyNameForm">
								<input id="profileName" name="profileName" type="text" placeholder="Profile name...">
						...
						<div class="sectionHeader" id="ColorSection"><p class="title">Color</p><div class="directInfo" style="background-color: #35a7ff"></div>
					</div>
					<div class="sectionContent" id="contentColor">
						<p>Choose your color</p>
						<div id="modifyColorForm">
	...
	$(document).ready(function(){
		$('.AddProfileView .scalerContainer').droppable({
			...
			drop : function(event, ui){
				...
				var nbProfiles = $('.owl-carousel > *').length;
				if (nbProfiles > 3){
					var addProfileHelper = $(this).closest('.item');
					$('#addProfileHelper').append($(addProfileHelper));
				}
				$('.owl-carousel').owlCarousel({
					items : 3,
