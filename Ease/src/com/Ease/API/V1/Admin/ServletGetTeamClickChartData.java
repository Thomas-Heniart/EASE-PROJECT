...
public class ServletGetTeamClickChartData extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            datasets.add(click_average);
            Calendar current = Calendar.getInstance();
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(team.getSubscription_date());
            if (calendar.get(Calendar.YEAR) > EASE_FIRST_YEAR) {
                do {
                    ((JSONArray) click_average.get("data")).addAll(team.getAverageOfClick(calendar.get(Calendar.YEAR), calendar.get(Calendar.WEEK_OF_YEAR), sm.getHibernateQuery()));
                    for (int i = 1; i <= 7; i++)
                        labels.add(++days);
