...
import { Container, Form, Message, Label, Button, Input, Icon, Checkbox } from 'semantic-ui-react';
  ...
  send = (e) => {
    ...
    if (this.props.profile_id === 0) {
      this.props.dispatch(createProfile({
        ...
        this.props.dispatch(catalogAddAnyApp({
      ...
      <Container>
        ...
        <Form onSubmit={this.send}>
          ...
          <div className='checkbox_credentials'>
            <Checkbox toggle onChange={this.handleInput} name='check' checked={this.state.check}/>
...
@connect(store => ({
  ...
  modal: store.teamModals.catalogAddAnyAppModal
}), reduxActionBinder)
class AddAnyAppModal extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      view: 1,
...
export default withRouter(AddAnyAppModal);