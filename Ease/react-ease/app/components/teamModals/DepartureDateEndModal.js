...
class DepartureDateEndModal extends Component {
  ...
  render(){
    ...
    return (
        <SimpleModalTemplate
            onClose={e => {this.props.dispatch(showDepartureDateEndModal(false))}}
          ...
          <Form class="container" error={this.state.errorMessage.length > 0} onSubmit={this.confirm}>
            <Form.Field>
            <strong class="capitalize">{user.username}</strong>’s account has been frozen because the departure date was set on {user.departure_date}. You can now decide to unfreeze the account or confirm the departure.
            </Form.Field>
